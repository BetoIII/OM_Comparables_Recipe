#!/usr/bin/env python3
"""
Generate interactive HTML viewer for comp sets
"""
...
if __name__ == "__main__":
    ...
    print(f"✅ Successfully generated HTML viewer!")
    print(f"📁 Comp Set: {result['comp_set_name']}")
    print(f"📊 Total Properties: {result['summary']['total_properties']}")
    print(f"✅ Properties with Full Data: {result['summary']['properties_with_full_data']}")
    print(f"⚠️  Properties with Missing Data: {result['summary']['properties_with_missing_data']}")
    print(f"🌐 HTML File: {result['html_file_path']}")
    
    if result['properties_with_missing_data']:
        print("\n⚠️  Properties with missing data:")
        for prop in result['properties_with_missing_data']:
            print(f"   - {prop['name']} ({prop['id']})")
    
    print(f"\n🚀 Open the HTML file in your browser to view the interactive comp set!")
    print(f"💡 Make sure the server is running (node server.js) to use editing features.")
